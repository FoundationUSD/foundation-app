...
export function ExternalVaultCard({ vault }: ExternalVaultCardProps) {
  ...
  return (
    <Link href={`/protocol/${vault.slug}`}>
      <div className="glass-card group relative cursor-pointer overflow-hidden p-5 transition-all hover:border-white/[0.15]">
        {/* Protocol badge */}
        <div className="mb-4 flex items-center justify-between">
          <span
            className={`${config.bg} ${config.color} rounded px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.15em]`}
          >
            {config.label}
          </span>
          ...
          {vault.tvl > 0 && (
            <div>
              <p className="font-mono text-sm text-foreground">
                {formatUSDCCompact(vault.tvl * 1_000_000)}
              </p>
              <p className="font-mono text-[9px] uppercase tracking-[0.15em] text-muted-foreground">
                TVL
              </p>
        ...
        {/* Action */}
        <div className="btn-glass flex w-full items-center justify-center gap-2 text-center">
          Deposit <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1" />
        </div>
      </div>
    </Link>
  );
